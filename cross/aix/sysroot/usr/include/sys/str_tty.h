/* @(#)06	1.28  src/bos/kernel/sys/str_tty.h, sysxcommon, bos53A, a2004_31C3 7/28/04 11:15:47 */
/*
...
#define		sysinfo_add(x,y)	fetch_and_addlp(&(x), (y))
#else
...
enum dds_type {             /* Which DDS type */
	...
	UC_SJIS_DDS,            /* sjis upper converter module */
        ...
        CXPA_ADAP_DDS,          /* cxpa PCI driver (for adapters) */
        CXPA_LINE_DDS,          /* cxpa PCI driver (for lines) */
        VCON_LINE_DDS,          /* LPAR Virtual console (Lines) */
        VCON_ADAP_DDS,          /* LPAR Virtual console (Adapters) */
        JAS_LINE_DDS,           /* Jasmine 2-port driver (Lines) */
        JAS_ADAP_DDS            /* Jasmine 2-port driver (Adapters) */
...
enum module_names {
	...
	lion,			/* lion driver name			*/
        ...
        cxia,                   /* ISA 8 and 128 port driver name       */
        cxpa,                   /* PCI 8 and 128 port driver name       */
        vcon,                   /* LPAR Virtual Console Driver name     */
        jas                     /* Jasmine 2-port driver name           */
...
struct	str_module_conf {
 	...
 	char	type;			/* type of the module :		*/
					...
					/* 's' for a stream head.	*/
	union {
		int	(*func_ptr)();	/* address of the module/driver */
					/* print function.		*/
		enum pdpf_type	func_num; /* pre-defined print function */
	} print_func;
  };

#define	print_funcptr	print_func.func_ptr
#define	print_funcnum	print_func.func_num

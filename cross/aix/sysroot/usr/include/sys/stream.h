/* @(#)38	1.7.2.37  src/bos/kernel/sys/stream.h, sysxpse, bos53H, h2006_02B5 12/16/05 04:06:17 */
/*
...
struct	msgb {
	...
	unsigned char	b_pad1;		/* emulation type and 7318 data */
					/* 7318 uses the low bit */
					/* UW7 compat uses the top bit */
	unsigned short	b_flag;		/* message flags */
	long		b_pad2;
	MSG_KERNEL_FIELDS
};
typedef	struct msgb	mblk_t;
#define b_emul	b_pad1			/* Setup b_emul for UW7 compat */
...
#define	MSG64BIT	0x0800	/* 64-bit message flag */
#define MSGIA32BIN	0x1000	/* process is an IA-32 binary */
#define	MSGATTR		0x8000	/* message contains security info */
...
struct	iocblk {
	...
	int32long64_t	ioc_rval;	/* return value */
...
struct copyreq {
	...
	int32long64_t	cq_flag;	/* reserved */
...
struct copyreq64 {
	...
	int32long64_t	cq_flag;	/* reserved */
	mblk_t *	cq_private;	/* module's private state info */
	ptr64		cq_addr64;	/* 64-bit address to copy data */
	long		cq_filler[2];
};
#endif	/* _KERNEL */
...
struct copyresp {
	...
	int32long64_t	cp_pad2;	/* reserved */
...
#define M_TTY		0x12	/* Reserved for tty internal use */
...
#define M_PCTTY		0x99	/* Reserved for tty internal use */
#define M_COPYIN64	0x9a	/* Module's request to perform 64-bit copyin */
#define M_COPYOUT64	0x9b	/* Module's request to perform 64-bit copyout */
#define M_XTIORDREL     0x9c    /* Internal msg: wake osrq for T_ORDREL_REQ */
...
struct	stroptions {
	__ulong32_t	so_flags;	/* options to set */
	short		so_readopt;	/* read option */
	ushort		so_wroff;	/* write offset */
	int32long64_t	so_minpsz;	/* minimum read packet size */
	int32long64_t	so_maxpsz;	/* maximum read packet size */
	int32long64_t	so_hiwat;	/* read queue high-water mark */
	int32long64_t	so_lowat;	/* read queue low-water mark */
	unsigned char	so_band;	/* band for water marks */
};

/* definitions for so_flags field */
#define	SO_ALL		(~0)	   /* Update all options */
#define	SO_READOPT	0x00000001 /* Set the read mode */
#define	SO_WROFF	0x00000002 /* Insert an offset in write M_DATA mblks */
#define	SO_MINPSZ	0x00000004 /* Change the min packet size on sth rq */
#define	SO_MAXPSZ	0x00000008 /* Change the max packet size on sth rq */
#define	SO_HIWAT	0x00000010 /* Change the high water mark on sth rq */
#define	SO_LOWAT	0x00000020 /* Change the low water mark */
#define	SO_MREADON      0x00000040 /* Request M_READ messages */
#define	SO_MREADOFF     0x00000080 /* Don't gen M_READ messages */
#define	SO_ISTTY	0x00000100 /* Act as controlling tty */
#define SO_ISNTTY	0x00000200 /* Don't act as controlling tty */
#define SO_NDELON	0x00000400 /* Apply TTY semantics on ONDELAY */
#define SO_NDELOFF	0x00000800 /* Apply STREAMS semantics on ONDELAY */
#define	SO_TOSTOP	0x00001000 /* Stop on background writes */
#define	SO_TONSTOP	0x00002000 /* Don't stop on background writes */
#define	SO_BAND		0x00004000 /* Water marks are for a band */
#define	SO_MSGATTR	0x00008000 /* ask sth wq to provide security info */
#define SO_PTY		0x00010000 /* VSU PTY HANGUP mod */
...
/* Used for Streams tunable parameters */
#define NSTREVENT	20
#define STRTURNCNT	15
#define PSETHRESH	85
#define STRLOWTHRESH	90
#define STRMEDTHRESH	95
...
/* M_TTY and M_PCTTY sub-opcodes and stuff */
#define MTTY_VERSION	0x01
#define S_READGONE	0x01
#define S_IOCTLGONE	0x02
#define S_LETSPLAY	0x03
#define S_DONTPLAY	0x04
#define S_BACKWASH	0x05
#define S_BACKDONE	0x06
#define S_DRAINO	0x07
#define S_DRAIN_ACK	0x08

struct ttyblk {
	ushort	tty_ver;	/* version number */
	ushort	tty_len;	/* length of valid data */
	uint	tty_op;		/* sub-opcode, listed above */
};

struct drain_blk {
	ushort mod_id;		/* module id of issuing module */
	ushort count;		/* id of this drain request */
};
...
#define getmajor        major_num
#define getminor        minor_num
#define getemajor       major_num
#define geteminor       minor_num
#define makedevice      makedevno
...
extern	int32long64_t	pse_timeout(timeout_fcn_t, timeout_arg_t, int);
#define timeout(a,b,c)	pse_timeout((timeout_fcn_t)(a),(timeout_arg_t)(b),c)

#undef	untimeout
extern	void		pse_untimeout(int32long64_t);
#define untimeout	pse_untimeout

#undef timeoutcf
extern	int		pse_timeout_cf(long);
...
/* returns 64-bit queue_t pointer for I_FDINSERT ioctl */
#define fdinsert_q64p(mp, offset)	*(queue_t **)(mp->b_rptr + offset)
#endif

extern  int             pse_sleep(tid_t *, int32long64_t);
extern  int             pse_sleepl(lock_t *, tid_t *, int32long64_t);
extern  int             pse_sleepx(int32long64_t, int, int32long64_t);
extern  int             pse_sleep_thread(tid_t *, void *, int32long64_t);

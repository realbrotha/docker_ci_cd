...
/*                                                                        */
/* bos53D src/bos/kernel/sys/dlpi.h 1.8                                   */
/*                                                                        */
...
/*                                                                        */
/* IBM_PROLOG_END_TAG                                                     */
/* @(#)41     1.8  src/bos/kernel/sys/dlpi.h, sysxdlpi, bos53D, d2005_22A1 4/28/05 02:12:28 */
/*
...
/*
 * Primitive to set on/off the Packet capture at LLC layer.UNDOCUMENTED.
 */
#define DL_TRACE_REQ            0x36    /* Request to turn ON/OFF packet capture */

/*
 * Timer Events for LLC-TYPE2. We use dl_unitdata_ind_t.
 * As of now we are only tracing the Timer expirations.
 * We may add to this list if additional events need to be traced. 
 */
#define DL_T1_TIMER_EXPIRED     0x01    /* T1 timer for LLC. Poll and Re-transmission timer*/
#define DL_T2_TIMER_EXPIRED     0x02    /* T2 timer for LLC. sent-REJ/Delayed ack timer    */
#define DL_TI_TIMER_EXPIRED     0x03    /* TI timer for LLC. Link Idle timer               */
...
/* DLPI Packet tracing related error codes */
#define DL_PPA_TRACE_ON 0x1c    /* per-PPA trace already ON */
#define DL_PPA_NOT_ACTIVE 0x1d  /* User tried to run a per-PPA trace on a non-exsitent PPA */
#define DL_SAP_TRACE_ON 0x1e    /* per-SAP trace for this SAP is already ON */
#define DL_SAP_NOT_ACTIVE 0x1f  /* User tried SAP tracing for not an active connected SAP */
#define DL_DLB_TRACE_ON 0x20    /* per-CONNECTION trace is already ON */
#define DL_DLB_TRACE_NOT_ACTIVE 0x21 /* User tried to trace a connection which is not created */
#define DL_PPA_TRACE_OFF 0x22   /* per-PPA trace is not ON */
#define DL_SAP_TRACE_OFF 0x23   /* per-SAP trace is not ON */
#define DL_DLB_TRACE_OFF 0x24   /* per-CONNECTION trace is not ON */
...
typedef struct {
	...
	t_uscalar_t	dl_sap_length;		/* current length of SAP part 
						   of dlsap address */
	...
	t_scalar_t	dl_provider_style;	/* style1 or style2 */
...
typedef struct {
	...
	t_uscalar_t	dl_correct_primitive;/* primitive being acknokwledged */
} dl_ok_ack_t;
...
typedef struct {
	...
	t_uscalar_t	dl_level;	/* physical,SAP level or ALLmulticast*/
} dl_promiscon_req_t;
...
typedef struct {
	...
	t_uscalar_t	dl_addr_length;		/* length of physical address */
...
typedef struct {
	...
	t_uscalar_t	dl_group_address;	/* set to one if multicase/broadcast */
} dl_unitdata_ind_t;
...
typedef struct {
	...
	t_uscalar_t	dl_service_class;	/* DL_RQST_RSP or DL_RQST_NORSP */
} dl_data_ack_req_t;
...
/*
 * DL_TRACE,M_PROTO type 
 */
typedef struct {
        t_uscalar_t     dl_primitive;
        t_uscalar_t     dl_on_or_off;
        t_uscalar_t     dl_lsap;
        t_uscalar_t     dl_ppa;
        t_uscalar_t     dl_dsap;
...
#define DL_TRACE_REQ_SIZE       sizeof(dl_trace_req_t)
#endif /* _SYS_DLPI_H */

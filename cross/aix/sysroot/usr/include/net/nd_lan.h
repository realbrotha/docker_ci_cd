...
/*                                                                        */
/* bos53L src/bos/kernel/net/nd_lan.h 1.15                                */
/*                                                                        */
...
/*                                                                        */
/* IBM_PROLOG_END_TAG                                                     */
/* @(#)12       1.15  src/bos/kernel/net/nd_lan.h, sysnet_cdli, bos53L, l2007_07B0 2/14/07 11:46:46  */
/*
...
#define DMXSTAT_INCR(stat, amt)	\
	fetch_and_add((int *)&(stat), (amt));
...
#define IFSTUFF_AND_DELIVER(nsuser, nddp, m, macp, extp) {		\
	...
	if (nsuser->user.ifp) {						\
		if ((nsuser->user.ifp->if_flags & IFF_UP) == 0) {	\
			m_freem(m);					\
			DMXSTAT_INCR(nsuser->user.ifp->if_iqdrops,1);	\
			goto out;					\
		} else {						\
			DMXSTAT_INCR(nsuser->user.ifp->if_ipackets,1);\
			DMXSTAT_INCR(nsuser->user.ifp->if_ibytes,m->m_pkthdr.len);	\
	...
	if (nsuser->user.protoq == NULL) {				\
		...
		if (IF_QFULL(nsuser->user.protoq)) {			\
			...
			DMXSTAT_INCR(nddp->ndd_demuxer->nd_dmxstats.nd_nobufs,1);	\
			m_freem(m);					\
		} else {						\
...
#define DEMUXER_LOCKINIT(lockp) {					\
	lock_alloc(lockp, LOCK_ALLOC_PIN, DEMUXER_LOCK_FAMILY, (uint)lockp);\

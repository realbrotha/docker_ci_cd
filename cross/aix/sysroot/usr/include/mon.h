...
/*                                                                        */
/* bos53Q src/bos/usr/include/mon.h 1.24                                  */
/*                                                                        */
...
/*                                                                        */
/* IBM_PROLOG_END_TAG                                                     */
/* @(#)55       1.24  src/bos/usr/include/mon.h, cmdstat, bos53Q, q2008_08B3 1/2/08 05:38:03 */
#ifndef _H_MON
#define _H_MON
/*
 ...
 */
#define MON_OUT  "mon"    /* -p profiling */
#define GMON_OUT "gmon"   /* -pg profiling */
...
#ifdef __ia64
#define MIN_SUBR_SPAN 16
#define INST_MIN_LENGTH 8
#else
#ifdef _POWER
 ...
 *
 * note that this structure serves for both 32- and 64-bit compilation
 * modes, and must therefore agree with struct prof64 in user.h
 */
struct prof {
...
/* output data file program loader buffer, for 32bit to read 64bit */
struct pldr64 {
        uint next;              /* offset to next element */
        unsigned long long textaddr; /* loaded text address */
        char names[2];          /* null terminated prog name path & */
};
...
/* output data file for range buffer, for 32bit to read 64bit */
struct outrng64 {
	unsigned long long lpc;
	unsigned long long hpc;
	int  nhcnt;
};
...
/* for 32bit to read 64bit */
struct poutcnt64 {
	unsigned long long fnpc;
...
struct goutcnt {                    /* tag was rawarc in BSD43 */
    ...
    unsigned long       raw_count;  /* to - from arc count */
};


/* 32bit to 64bit */
struct goutcnt64 {
	unsigned long long raw_frompc;
	unsigned long long raw_selfpc;
	long long          raw_count;
};

/* output data file thread level function call count buffer array element -pg profiling */
struct goutthrdcnt {
...
struct tostruct {
    ...
    unsigned long       count;   /* arc count */
    HASH_LINK           link;    /* next element in chain index */
    };

/* 32bit to 64bit */
struct tostruct64 {
    unsigned long long  selfpc;  /* to subroutine address */
    long long           count;   /* arc count */
    HASH_LINK           link;    /* next element in chain index */
    };
...
struct monglobal {
    ...
    ulong            ProfLoPC;       /* pc offset for hist buffer - lo limit */
    ...
    caddr_t mon_graph_hash; /* used by monitor to free if not NULL */
    } ;
...
#ifndef _NO_PROTO
void profil(void *bufbase, uint bufsize, ulong pcoffset, uint pcscale);
#else
void profil();
#endif

/* Used by threads library to get the historgram buffer */
#ifndef _NO_PROTO
caddr_t getthreadbuffer(unsigned long thread_id, char type);
#else
caddr_t getthreadbuffer();
#endif

/* Used by threads library to free the obtained thread buffer */
#ifndef _NO_PROTO
void freethreadbuffer(unsigned long thread_id, char type);
#else
void freethreadbuffer();
#endif

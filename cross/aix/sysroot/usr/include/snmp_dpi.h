...
/*                                                                        */
/* tcpip530 src/tcpip/usr/include/snmp_dpi.h 1.3                          */
/*                                                                        */
...
/*                                                                        */
/* IBM_PROLOG_END_TAG                                                     */
/* @(#)28	1.3  src/tcpip/usr/include/snmp_dpi.h, snmp, tcpip530 5/4/00 16:19:33 */
/*
\begin{verbatim}
*/
...
/*                                                                   */
/* Feb 17, 1999 - Version 2.0.31                                     */
/*                                                                   */
/* Copyright    - (C) International Business Machines Corp. 1994-1999*/
/*                                                                   */
...
/*   snmp_dpi_u64 structure that has 2 unsigned int integers, one    */
...
/*                                                                   */
/* Notes/Warnings:                                                   */
/* - #defines for UInteger32, NsapAddress and BIT_STRING below are   */
/*   here just in case someone used them in the past.                */
/*   However, current master agents do not accept those types.       */
/*   They are obsolete and only were used in RFC1442 which is now    */
/*   Historic. You better do not use them in new code.               */
/*                                                                   */
/*********************************************************************/
...
struct dpi_reg_packet {
  ...
  int                     priority;    /* priority for this subagent */
...
struct dpi_bulk_packet {
  int                     non_repeaters;  /* number of non-repeaters */
  int                     max_repetitions;/* max repeaters           */
  struct dpi_next_packet *varBind_p;      /* ptr to varBinds: chain  */
                                          /* of dpi_next_packets     */
};

struct snmp_dpi_u64 {                  /* for unsigned 64-bit int    */
  unsigned int high;                   /* - high order 32 bits       */
  unsigned int low;                    /* - low order  32 bits       */
...
struct dpi_resp_packet {
  ...
  unsigned int             error_index;/* index 1st varBind in error */
...
struct dpi_trap_packet {
  int                      generic;    /* long must be 4 bytes long  */
  int                      specific;
  struct dpi_set_packet   *varBind_p;  /* ptr to varBinds: chain of  */
                                       /* SET structures             */
...
unsigned char * LINKAGE mkDPIbulk(     /* Make a DPI response packet */
  int                  non_repeaters,  /* non repeaters              */
  int                  max_repetitions,/* maximum repetitions        */
...
unsigned char * LINKAGE mkDPIresp(     /* Make a DPI response packet */
  ...
  int                   ret_code,      /* error code: SNMP_ERROR_xxx */
  int                   ret_index,     /* index to varBind in error  */
  snmp_dpi_set_packet  *packet_p);     /* ptr to chain of SET packets*/
                                       /* containing the varBinds    */

unsigned char * LINKAGE mkDPItrape(    /* Make a DPI trap packet     */
  int                   generic,       /* generic trap type  (32 bit)*/
  int                   specific,      /* specific trap type (32 bit)*/
  snmp_dpi_set_packet  *packet_p,      /* ptr to chain of SET packets*/
                                       /* containing the varBinds    */
...
#define DPI_RC_UNSUPPORTED_DOMAIN  -9  /* unsupported transportDomain*/
#define DPI_RC_INVALID_ARGUMENT    -10 /* invalid argument detected  */
...
int LINKAGE  DPIconnect_to_agent_UNIXstream( /* Connect  to DPI via  */
                                             /* a UNIX stream        */
  char                 *hostname_p,    /* target hostname/IPaddress  */
  char                 *community_p);  /* communityname              */
...
int LINKAGE  DPIawait_packet_from_agent(  /* await a DPI packet      */
  ...
  unsigned int         *length);       /* receives length of data    */
...
#ifndef DPI_MINIMAL_SUBAGENT
int  LINKAGE     qDPIport(             /* Query (GET) SNMP_DPI port  */
  char                 *hostname_p,    /* target hostname/IPaddress  */
...
#ifndef SNMP_VERY_MINIMAL_AGENT
unsigned int   LINKAGE  lookup_host(   /* find IPaddress in network  */

...
/*                                                                        */
/* bos530 src/bos/usr/ccs/lib/libpam/security/pam_appl.h 1.3              */
/*                                                                        */
...
/*
 * gets PAM environment variable values based upon the variable's name.
 * if the variable is not found, a NULL pointer is returned.
 * if the variable is found, the string representation of its value
 * 	is returned.
 * the caller is responsible for deallocating the returned value.
 */
extern char *
pam_getenv(
	pam_handle_t	*pamh,
...
/*
 * sets the value of a PAM environment variable.
 * if a variable of that name already exists, then its value is replace
 * 	with the new value.
 * else, the variable and its value are added to the set of current PAM
 * 	environment variables.
 *
 * the parameter "namevalue" is of the form <VARIABLE>=<VALUE>.
 */
extern int
pam_putenv(
	pam_handle_t	*pam_h,
...
/*
 * returns a list of the currently set PAM environment variables.
 * the caller is responsible for deallocating the returned list.
 *
 * if no PAM environment variables are set, a NULL pointer is returned.
 */
extern char **
pam_getenvlist(
	pam_handle_t	*pam_h
...
/*
 * Stubbed routine which will always return a PAM_SYSTEM_ERR error.
 */
extern int
